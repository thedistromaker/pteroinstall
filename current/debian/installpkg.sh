...
if [ -n $PHP ]; then
    ...
    apt-get install -y php8.3 > /dev/null
    ret=$?
    if [ $ret -ne 0 ]; then
...
if [ -n $BASIC ]; then
    ...
    cat .basic.missing | cut -c 6- | xargs apt-get install -y > /dev/null
    ret=$?
    if [ $ret -ne 0 ]; then
...
if [ -n $PHPEXT ]; then
    ...
    cat .ext.missing | cut -c 6- | xargs apt-get install -y > /dev/null
    ret=$?
    if [ $ret -ne 0 ]; then
...
if [ -n $PHPFPM ]; then
    ...
    apt-get install -y php8.3-fpm > /dev/null
    ret=$?
    if [ $ret -ne 0 ]; then
